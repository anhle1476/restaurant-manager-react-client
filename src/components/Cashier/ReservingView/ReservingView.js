...
import { toastErrorLeft } from "../../../utils/toastUtils";

import "react-datepicker/dist/react-datepicker.css";
import "./ReservingView.scss";

const NEW_RESERVING_ORDER_SCHEMA = {
...
const ReservingView = () => {
  ...
  const handleCreateReservingOrder = async (e) => {
    e.preventDefault();
  };
  ...
  return (
    <div className="flex-container">
              ...
              {reservingOrders.length ? (
                reservingOrders.filter(searchFilter).map((order) => (
                  <tr key={order.id}>
                    ...
                    <td>
                      {order.appTables.map((table) => table.name).join(", ")}
                    </td>
      ...
      <div className="flex-footer">
        <Button block disabled={daysFromToday > 0} color="warning">
